...
#include "audio_utils/include/audio_utils/primitives.h"
    ...
    while (remaining >= 8) {
        ...
        if (!memcmp(&chunk[0], "fmt ", 4)) {
            ...
            switch (format) {
                ...
                default:
#ifdef HAVE_STDERR
                   // LOG("unsupported format %u\n", format);
#endif
                    goto close;
            }
            if (chunkSize < minSize) {
#ifdef HAVE_STDERR
               // LOG("chunkSize %u < minSize %zu\n", chunkSize, minSize);
#endif
                goto close;
            }
            ...
            if (bitsPerSample != 8 && bitsPerSample != 16 && bitsPerSample != 24 &&
                bitsPerSample != 32) {
#ifdef HAVE_STDERR
                //LOG("bitsPerSample %u != 8 or 16 or 24 or 32\n", bitsPerSample);
#endif
                goto close;
            }
...
#ifdef HAVE_STDERR
            //LOG("ignoring unknown chunk %c%c%c%c\n",
...
//    if (remaining > 0) {
//#ifdef HAVE_STDERR
//        LOG("partial chunk at end of RIFF, remaining %zu\n", remaining);
//#endif
//        goto close;
//    }
    if (!hadData) {
